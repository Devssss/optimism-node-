import { NextResponse } from 'next/server';

export async function GET() {
  const ROOT_URL = process.env.NEXT_PUBLIC_URL || 'https://example.com';
  
  const manifest = {
    accountAssociation: {
      header: "eyJmaWQiOjkxNTIsInR5cGUiOiJjdXN0b2R5Iiwia2V5IjoiMHgwMmVmNzkwRGQ3OTkzQTM1ZkQ4NDdDMDUzRURkQUU5NDBEMDU1NTk2In0",
      payload: "eyJkb21haW4iOiJhcHAuZXhhbXBsZS5jb20ifQ",
      signature: ""
    },
    miniapp: {
      version: "1",
      name: "OP Node Tracker",
      homeUrl: ROOT_URL,
      iconUrl: `${ROOT_URL}/icon.png`,
      splashImageUrl: `${ROOT_URL}/splash.png`,
      splashBackgroundColor: "#050505",
      webhookUrl: `${ROOT_URL}/api/webhook`,
      subtitle: "Monitoring Optimism Stack Nodes",
      description: "Real-time stats and health monitoring for Optimism-stack blockchain nodes.",
      screenshotUrls: [],
      primaryCategory: "utility",
      tags: ["blockchain", "optimism", "node", "base"],
      heroImageUrl: `${ROOT_URL}/hero.png`,
      tagline: "Track your node health",
      ogTitle: "OP Node Tracker",
      ogDescription: "Real-time monitoring for OP nodes",
      ogImageUrl: `${ROOT_URL}/hero.png`,
      noindex: true
    }
  };

  return NextResponse.json(manifest);
}
